...
def _search_recids(query):
    return [int(recid) for recid in list(search_pattern(p=query))]


def search_dups(collection_query):
    recids_str = ",".join(str(recid) for recid in _search_recids(collection_query))
...
def search_ep_report_number_dups(collection_query):
    ...
    """
    ep_map = {}
    for recid in _search_recids(collection_query):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "-980:HIDDEN -980__c:MIGRATED -980__a:DUMMY."
        ),
    )
