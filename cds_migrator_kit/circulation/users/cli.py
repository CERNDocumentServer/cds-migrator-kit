...
def users(users_json):
    ...
    def _import_users(users, users_identities, users_profiles, remote_accounts):
        """Import users in db."""
        ...
        click.secho(
            "Migrating {0} user identities".format(len(users_identities)), fg="green"
        )
        ...
        click.secho(
            "Migrating {0} user profiles".format(len(users_profiles)), fg="green"
        )
        ...
        click.secho(
            "Migrating {0} remote accoutns".format(len(remote_accounts)), fg="green"
        )
