...
class LibrariesMigrator:
    ...
    def __init__(self, libraries):
        ...
        self.libraries = [
            l
            for l in libraries
            if l["type"] in self.VALID_LIBRARY_TYPES
            and l["id"] not in self.IGNORE_LIBRARY_IDS
        ]
...
class ItemsMigrator:
    ...
    def _clean_date(self, legacy_date, item):
        ...
        if not legacy_date:
            _log = (
                "`creation_date` or `modification_date` is None for item "
    ...
    def migrate(self):
        ...
        for i, item in enumerate(self.items):
            ...
            if not item["barcode"]:
                _log = (
                    "Item with `id_bibrec = {id_bibrec}` not imported "
                    "because `barcode is None`".format(id_bibrec=item["id_bibrec"])
                )
                logger.error(_log)
                print(_log)
                continue
            ...
            if item["id_crcLIBRARY"] not in self.internal_locations:
                _log = (
                    "Item with `barcode = {barcode}` not imported because"
            ...
            try:
                ...
                _log = (
                    ...
                    " `loan_period={loan_period}` not recognized".format(
                        barcode=item["barcode"], loan_period=item["loan_period"]
                    )
                )
                logger.error(_log)
                print(_log)
