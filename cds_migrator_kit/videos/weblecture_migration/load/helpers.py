...
def copy_file_to_bucket(bucket_id, file_path, is_master=False):
    ...
    try:
        ...
        with open(file_path, "rb") as file_stream:
            checksum_original = file_storage._compute_checksum(stream=file_stream)
        if checksum_original != file_checksum:
            error_message = (
...
def transcode_task(payload, subformats):
    ...
    for task in flow_tasks:
        ...
        if preset_quality in added_qualities:
            ...
            # TODO it'll update the status as FAILURE if missing
