...
class TransformFiles:
    ...
    def _check_presenter_presentation_exists(self, file_paths):
        """Check the folder has presenter and presentation files."""
        keywords = {"presenter", "presentation"}
        found = set()

        for file_path in file_paths:
            file_name = Path(file_path).stem.lower()  # Extract file name without extension
            for keyword in keywords:
                if keyword in file_name:
                    found.add(keyword)
            
            if found == keywords:  # If both keywords are found, return True
    ...
    def _find_master_data_folder_path(self, path):
        ...
        # Try to find files in folder
        for search_path in search_paths:
            all_files = self._get_all_files_in_folder(search_path)
            if all_files:
                return search_path# Files are found
            
        # No files are found, raise an error
        raise MissingRequiredField(
            message="Master file folder couldn't be found!",
            stage="transform",
            recid=self.recid,
            value="master_path",
            priority="critical",
    ...
    def _check_master_data_folder(self):
        ...
        # Presenter and presentation doesn't exists, try to find the file named "video.mp4"
        video_files = [file for file in all_files if "video" in file.lower()]
    ...
    def _frames_exists(self, frames_list):
        ...
        raise ManualImportRequired(
            message="Missing or extra frames!",
            stage="transform",
            recid=self.recid,
            value="frames",
            priority="critical",
    ...
    def _get_subformats_from_datav2json(self, data):
        ...
        try:
            subformats_list = [file.get("src").strip("/") for file in data.get("streams", [])[0].get("sources", {}).get("mp4", [])]
        except:
            raise ManualImportRequired(
    ...
    def _get_highest_and_other_composites(self):
        ...
        other_composites = [composite_videos[res] for res in [1080, 720, 480, 360] if res in composite_videos]
    ...
    def _check_files_in_media_data_folder(self, files_paths):
        ...
        # Get all files in media_data folder
        all_files = self._get_all_files_in_folder(self.media_data_folder)
    ...
    def _set_media_files(self, files_paths):
        ...
        if self.use_composite:
            ...
            additional_files = [file for file in files_paths 
                if "presenter" not in Path(file).stem.lower() 
                and "presentation" not in Path(file).stem.lower()]
            self._add_files_to_file_json(
                json_key="additional_files",
        ...
        # We dont have the composite we'll get the subformats and frames from the folder
            ...
            # ~~~~SUBFORMATS~~~~
            subformats_list = self._get_subformats_from_datav2json(data)
